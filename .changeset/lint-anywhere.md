...
- The Prompt API harness is its own lazy fixture, `promptApi`, instead of `webmcp.promptApi`, and `toPassEval` takes an agent as its receiver (`expect(promptApi).toPassEval(...)`). New `defineAgent(webmcp, drive)`, `toolsForAgent()` and `evaluateAgent()` let any Node-side model or agent framework drive the page's tools with calls recorded as agent calls; `webmcp.call()` accepts `via: "agent"`. Codegen renders the `promptApi` fixture.

...
- `@swissspidy/eslint-plugin-webmcp`: the tool-scoped rules as ESLint rules for `registerTool()`, `useWebMCP()`, your own wrappers and `<form toolname>` in JSX, plus source rules for the API itself (`no-legacy-api`, `no-unknown-tool-properties`, `no-unknown-options`, `require-execute`, `valid-event-name`, `no-unbound-method`, `no-interpolated-text`); loads in oxlint.

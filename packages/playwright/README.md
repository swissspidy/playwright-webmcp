...
```sh
npm install --save-dev @swissspidy/playwright-webmcp @swissspidy/webmcp-lint @playwright/test
```
...
Suite-wide reports (`tools.json` for `webmcp-evals local`, `coverage.json`, `TOOLS.md`) come from the bundled reporter: `reporter: [["list"], ["@swissspidy/playwright-webmcp/reporter", { outputDir: ".webmcp-report" }]]`.
